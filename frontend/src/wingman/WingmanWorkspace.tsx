...
export default function Workspace({
  ...
  const role: Role = ["owner", "mechanic", "warehouse", "admin"].includes(
    providedRole,
  )
    ? providedRole
    : "owner";
  ...
  useEffect(() => {
    document.documentElement.dataset.palette = "pearl";
    return () => {
  ...
  useEffect(() => {
    if (role !== "owner" || !authenticated) return;
    getOwnerNotifications()
  ...
  async function openNotification(notification: any) {
    if (!notification.read_at) {
      try {
        ...
        setToast("The notification could not be marked as read.");
      }
    }
    go("Vehicle Care");
    setModal("");
  ...
  return (
    <div className={"workspace " + (role === "owner" ? "owner-workspace" : "")}>
      <div className="glass-atmosphere" aria-hidden="true">
        <span className="glass-orb one" />
        <span className="glass-orb two" />
        <span className="glass-orb three" />
      </div>
        ...
        <nav className="side-nav">
          {items.map(([label, Icon]) => (
            ...
            >
              ...
              <span>{label}</span>
              {label === "My requests" && <small>1</small>}
              {label === "Messages" && <span className="nav-dot" />}
              {label === "Approvals" && <small>2</small>}
            </button>
          ))}
        </nav>
        <div className="sidebar-bottom">
          <button className="sidebar-user" onClick={() => setModal("account")}>
            <span className="avatar">{initials}</span>
            <div>
              <strong>{name}</strong>
              <small>Wingman account</small>
            </div>
            <ChevronDown size={15} />
          </button>
        </div>
      ...
      <div className="workspace-main">
        ...
        <footer className={"workspace-disclaimer " + (paused ? "paused" : "")}>
          <span>DEMO DIRECTORY</span>
          <div>
            Sample businesses, availability, stock, prices, and activity. No
            live provider participation or roadside dispatch.
          </div>
          <button
            onClick={() => setPaused(!paused)}
            aria-label={paused ? "Resume disclaimer" : "Pause disclaimer"}
          >
            {paused ? "Resume" : "Pause"}
          </button>
        </footer>
      ...
      {modal === "switch" && (
        <Modal
          title="One network. Four perspectives."
          subtitle="Explore how Wingman works for everyone involved."
          onClose={() => setModal("")}
        >
          <div className="switch-roles">
            {(["owner", "mechanic", "warehouse", "admin"] as Role[]).map(
              (r) => (
                <a href={"/demo/" + r} key={r}>
                  ...
                  <div>
                    ...
                    <small>
                      {r === "owner"
                        ? "Find help and care for your vehicle"
                        : r === "mechanic"
                          ? "Manage jobs, parts, and appointments"
                          : r === "warehouse"
                            ? "Keep inventory and orders moving"
                            : "Oversee the network and its people"}
                    </small>
                  </div>
            ...
            {role === "owner" && authenticated ? (
              ...
              : [
                ...
                <button
                  key={title}
                  onClick={() => {
                    go(role === "admin" ? "Alerts" : destination);
                    setModal("");
                  }}
                >
                  <span className="icon-disc">
      ...
      {modal === "about" && (
        <Modal
          title="About Wingman"
          subtitle="One connected roadside workspace"
          onClose={() => setModal("")}
        >
          <p>
            Explore the owner, mechanic, warehouse, and administrator
            experiences in one connected system.
          </p>
          <button className="primary" onClick={() => setModal("switch")}>
            Explore workspaces <ArrowUpRight size={17} />
          </button>
        </Modal>
      )}
    </div>
